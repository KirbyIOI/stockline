...
router.get("/", (req, res) => {
  ...
  const rows = status
    ? db.prepare(SELECT_JOIN + "WHERE o.status = ? ORDER BY o.placed_at DESC").all(status)
    : db.prepare(SELECT_JOIN + "ORDER BY o.placed_at DESC").all();
  res.json(rows.map(toOrderShape));
});

// POST /api/orders — place a purchase order for a product { productId, qty, supplierName }
router.post("/", (req, res) => {
  const { productId, qty, supplierName, source } = req.body;
...
router.patch("/:id/cancel", (req, res) => {
  ...
  const row = db.prepare(SELECT_JOIN + "WHERE o.id = ?").get(req.params.id);
...
router.patch("/:id/receive", (req, res) => {
  ...
  const row = db.prepare(SELECT_JOIN + "WHERE o.id = ?").get(order.id);
  res.json(toOrderShape(row));
});
